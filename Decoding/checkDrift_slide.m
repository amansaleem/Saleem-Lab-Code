function [Posterior_all] = checkDrift_slide(Posterior_all, slideBins, cal_diffs, cal_var, plot_flag)
if nargin<2
    slideBins = 5;
end
if nargin<3
...
if Posterior_all(1).decoder.numBins==50
    X = -50:50;
    lowD = 1;
    highD = 20;
    ...
    numBins = Posterior_all(1).decoder.numBins;
    X = -numBins:numBins;
    lowD = 1;
    highD = round(40*(numBins/100));
    drange = [(numBins-highD):(numBins+1)-lowD (numBins+1)+lowD:((numBins+1)+highD)];
    drange2= [1:(numBins+1)-lowD (numBins+1)+lowD:(numBins*2)];
    smth_win = 0;
    gradient_n = 1;
    diff_amount = 1;
    
    low_lim = ceil(10*(numBins/100));
    high_lim = floor(65*(numBins/100));
    posBins = [1:round(5*numBins/100):numBins];
end
...
for animalIdx = 1:length(Posterior_all)
    ...
    if cal_diffs
        ...
        diffs(animalIdx).low(diffs(animalIdx).X_low<0) = nan;
        diffs(animalIdx).X_low(diffs(animalIdx).X_low<0)=nan;
        diffs(animalIdx).norm(diffs(animalIdx).X_norm<0) = nan;
        diffs(animalIdx).X_norm(diffs(animalIdx).X_norm<0)=nan;
        diffs(animalIdx).high(diffs(animalIdx).X_high<0) = nan;
        diffs(animalIdx).X_high(diffs(animalIdx).X_high<0)=nan;
    else
        diffs(animalIdx).low  = Posterior_all(animalIdx).MAP.low;%  - circshift(Posterior_all(animalIdx).MAP.low', diff_amount)';
        diffs(animalIdx).norm = Posterior_all(animalIdx).MAP.norm;% - circshift(Posterior_all(animalIdx).MAP.norm',diff_amount)';
        diffs(animalIdx).high = Posterior_all(animalIdx).MAP.high;% - circshift(Posterior_all(animalIdx).MAP.high',diff_amount)';
    ...
    Posterior_all(animalIdx).slideVar = slideVar(animalIdx);
    ...
    % Using decoded position
%     X_low  = Posterior_all(animalIdx).MAP.low;
%     X_norm = Posterior_all(animalIdx).MAP.norm;
%     X_high = Posterior_all(animalIdx).MAP.high;
...
if plot_flag
    ...
    for animalIdx = 1:size(meanSlide.low,1)
        ...
        hold off
        plot(posBins(1:end-1)*(100/numBins), meanSlide.high(animalIdx,:),'r')
        hold on;
        plot(posBins(1:end-1)*(100/numBins), meanSlide.low(animalIdx,:),'b')
    ...
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.high,1),'r')
    hold on;
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.low,1),'b')
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    line([80 80],ylim, 'color','k', 'linestyle','--');
    ...
    figure(102)
    subplot(221)
    % errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanSlide.high,1),sem(meanSlide.high),'r')
    % hold on;
    % errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanSlide.low,1),sem(meanSlide.low),'b')
    % errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanSlide.norm,1),sem(meanSlide.norm),'k')
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.high,1),'r', 'linewidth',1.2)
    hold on;
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.low,1),'b', 'linewidth',1.2)
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.norm,1),'k', 'linewidth',1.2)
    
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.X_high,1),'r--', 'linewidth',1)
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.X_low,1),'b--', 'linewidth',2)
    plot(posBins(1:end-1)*(100/numBins), mean(meanSlide.X_norm,1),'k--', 'linewidth',2)
    axis tight
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    ...
    for animalIdx = 1:size(meanSlide.low,1)
        subplot(3,3,animalIdx)
        hold off
        plot(posBins(1:end-1)*(100/numBins), meanSlide.low(animalIdx,:) - meanSlide.high(animalIdx,:),'k')
        %     hold on; plot(posBins(1:end-1), meanSlide.norm(n,:),'k')
        %     plot(posBins(1:end-1), ,'r')
        axis tight
        line(xlim, [0 0], 'color','k', 'linestyle','--'); %axis([1 50 0 max([meanSlide.low(n,:) meanSlide.high(n,:)])]);
    ...
    subplot(3,3,9)
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanSlide.low-meanSlide.high,1), sem(meanSlide.low-meanSlide.high), 'r');
    hold on;
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanSlide.norm-meanSlide.high,1), sem(meanSlide.norm-meanSlide.high), 'k');
    ...
    for animalIdx = 1:size(meanconf.low,1)
        subplot(3,3,animalIdx)
        hold off
        plot(posBins(1:end-1)*(100/numBins), meanconf.high(animalIdx,:),'r')
        hold on;
        plot(posBins(1:end-1)*(100/numBins), meanconf.low(animalIdx,:),'b')
        line([20 20],ylim, 'color','k', 'linestyle','--');
        line([40 40],ylim, 'color','k', 'linestyle','--');
        line([60 60],ylim, 'color','k', 'linestyle','--');
        line([80 80],ylim, 'color','k', 'linestyle','--');
    ...
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanconf.low - meanconf.high,1),sem(meanconf.low - meanconf.high),'k')
    % hold on;
    % plot(posBins(1:end-1)*(100/numBins), mean(meanconf.low,1),'b')
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    line([80 80],ylim, 'color','k', 'linestyle','--');
    ...
    figure(101)
    subplot(221)
    plot(posBins(1:end-1)*(100/numBins), mean(meanconf.norm, 1),'k')
    hold on;
    plot(posBins(1:end-1)*(100/numBins), mean(meanconf.low, 1),'b')
    plot(posBins(1:end-1)*(100/numBins), mean(meanconf.high, 1),'r')
    % plot(posBins(1:end-1)*(100/numBins), mean(meanconf.low,1),'b')
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    ...
    subplot(222)
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanconf.norm - meanconf.low,1),sem(meanconf.norm - meanconf.low),'b')
    hold on;
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanconf.norm - meanconf.high,1),sem(meanconf.norm - meanconf.high),'r')
    % plot(posBins(1:end-1)*(100/numBins), mean(meanconf.low,1),'b')
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    ...
    subplot(223)
    hold off;
    errorarea_as(posBins(1:end-1)*(100/numBins), -mean(meanSlide.norm - meanSlide.low,1),sem(meanSlide.norm - meanSlide.low),'b')
    hold on;
    errorarea_as(posBins(1:end-1)*(100/numBins), -mean(meanSlide.norm - meanSlide.high,1),sem(meanSlide.norm - meanSlide.high),'r')
    hold off;
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    ...
    for animalIdx = 1:size(meanerr.low,1)
        subplot(3,3,animalIdx)
        hold off
        plot(posBins(1:end-1)*(100/numBins), meanerr.high(animalIdx,:),'r')
        hold on;
        plot(posBins(1:end-1)*(100/numBins), meanerr.low(animalIdx,:),'b')
        line([20 20],ylim, 'color','k', 'linestyle','--');
        line([40 40],ylim, 'color','k', 'linestyle','--');
        line([60 60],ylim, 'color','k', 'linestyle','--');
        line([80 80],ylim, 'color','k', 'linestyle','--');
    ...
    errorarea_as(posBins(1:end-1)*(100/numBins), mean(meanerr.low - meanerr.high,1),sem(meanerr.low - meanerr.high),'k')
    % hold on;
    % plot(posBins(1:end-1)*(100/numBins), mean(meanerr.low,1),'b')
    line([20 20],ylim, 'color','k', 'linestyle','--');
    line([40 40],ylim, 'color','k', 'linestyle','--');
    line([60 60],ylim, 'color','k', 'linestyle','--');
    line([80 80],ylim, 'color','k', 'linestyle','--');
