% function just loading all the es and saving them
idx = 0;
% smthWin = 100;
% box_filt = 1;
% trainCorrect = 1;
...
for idx = 1:length(expt_list)
   [es] = loadESwithTheta(expt_list(idx).animal, ...
        expt_list(idx).iseries, expt_list(idx).expt_list, expt_list(idx).shank_list);
   fileName = ['es_' expt_list(idx).animal, '_'...
        num2str(expt_list(idx).iseries)];
   save(['Data' filesep fileName], 'es');
   clear es
end
