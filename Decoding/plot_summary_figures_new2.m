...
if min(Posterior_all(1).meanPost.low(:))<0
    rescale = 1;
else
    rescale = 0;
end
...
if ~rescale
    for n = 1:length(Posterior_all)
          SummaryMaps.low(n,  :,:) = log2(50*Posterior_all(n).meanPost.low);
            SummaryMaps.high(n, :,:) = log2(50*Posterior_all(n).meanPost.high);
            SummaryMaps.norm(n, :,:) = log2(50*Posterior_all(n).meanPost.norm);
            SummaryMaps.low_orig(n,  :,:) = (Posterior_all(n).meanPost.low_orig);
            SummaryMaps.high_orig(n, :,:) = (Posterior_all(n).meanPost.high_orig);
        
            SummaryMaps.allcond.low.early(n,:,:)    = log2(50*Posterior_all(n).meanPost_new.low.early);
            SummaryMaps.allcond.low.correct(n,:,:)  = log2(50*Posterior_all(n).meanPost_new.low.correct);
            SummaryMaps.allcond.low.late(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.low.late);
            SummaryMaps.allcond.low.miss(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.low.miss);
        
            SummaryMaps.allcond.high.early(n,:,:)    = log2(50*Posterior_all(n).meanPost_new.high.early);
            SummaryMaps.allcond.high.correct(n,:,:)  = log2(50*Posterior_all(n).meanPost_new.high.correct);
            SummaryMaps.allcond.high.late(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.high.late);
            SummaryMaps.allcond.high.miss(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.high.miss);
        
            SummaryMaps.allcond.norm.early(n,:,:)    = log2(50*Posterior_all(n).meanPost_new.norm.early);
            SummaryMaps.allcond.norm.correct(n,:,:)  = log2(50*Posterior_all(n).meanPost_new.norm.correct);
            SummaryMaps.allcond.norm.late(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.norm.late);
            SummaryMaps.allcond.norm.miss(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.norm.miss);
        
            SummaryMaps.allcond.all.early(n,:,:)    = log2(50*Posterior_all(n).meanPost_new.all.early);
            SummaryMaps.allcond.all.correct(n,:,:)  = log2(50*Posterior_all(n).meanPost_new.all.correct);
            SummaryMaps.allcond.all.late(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.all.late);
            SummaryMaps.allcond.all.miss(n,:,:)     = log2(50*Posterior_all(n).meanPost_new.all.miss);
...
clear marginals
for n = 1:length(Posterior_all)
    tmp = sq(SummaryMaps.allcond.all.early(n,:,:));
    [Posterior_all(n).marginals.all.early, X, tmp] = ...
        get45Marginal(tmp,marBins);
    tmp = sq(SummaryMaps.allcond.all.correct(n,:,:));
    [Posterior_all(n).marginals.all.correct, X] = ...
        get45Marginal(tmp,marBins);
    norm = Posterior_all(n).marginals.all.correct(find(X == min(abs(X))));
    tmp = sq(SummaryMaps.allcond.all.late(n,:,:));
    [Posterior_all(n).marginals.all.late, X] = ...
        get45Marginal(tmp,marBins);
    tmp = sq(SummaryMaps.allcond.all.miss(n,:,:));
    [Posterior_all(n).marginals.all.miss, X] = ...
        get45Marginal(tmp,marBins);
    marginals.early(n,:)    = Posterior_all(n).marginals.all.early./norm;
    marginals.correct(n,:)  = Posterior_all(n).marginals.all.correct./norm;
    marginals.late(n,:)     = Posterior_all(n).marginals.all.late./norm;
    marginals.miss(n,:)     = Posterior_all(n).marginals.all.miss./norm;
end
X = X*2*sqrt(2);
tmp = X>-75 & X<75;
figure
errorarea_as(X(tmp), nanmean(marginals.early([1 2 4:7],tmp),1),nansem(marginals.early([1 2 4:7],tmp),[],1), 'r')
hold on;
errorarea_as(X(tmp), nanmean(marginals.correct(:,tmp),1),nansem(marginals.correct(:,tmp),[],1), 'g')
errorarea_as(X(tmp), nanmean(marginals.late(:,tmp),1),nansem(marginals.late(:,tmp),[],1), 'b')
% plot(X(tmp), nanmean(marginals.early([1 2 4:7],tmp),1), 'r','linewidth',2)
% hold on;
% plot(X(tmp), nanmean(marginals.correct(:,tmp),1), 'g','linewidth',2)
% plot(X(tmp), nanmean(marginals.late(:,tmp),1), 'b','linewidth',2)
set(gca, 'box','off','TickDir','out','fontsize',14,'color','none');
