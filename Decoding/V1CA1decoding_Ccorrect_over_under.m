...
%         % Shuffled
%         Vtemp =  V_ML_l(temp);
%         Vtemp = Vtemp(randperm(length(Vtemp)));
%         V_all.la = [V_all.la Vtemp'];
%         Vtemp =  V_ML_l(temp_s);
%         Vtemp = Vtemp(randperm(length(Vtemp)));
%         V_all.ls = [V_all.ls V_ML_l(temp_s)'];
%         Vtemp =  V_ML_l(temp_n);
...
%     F = smoothhist2D_corrected([V_all.ha' C_all.ha'],4,[50 50], 1:50);
    ...
    subplot(3,3,1)
    [F, c1, c2] = smoothhist2D_corrected([C_all.ls' X_all.ls'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.ls)/(length(X_all.ls)+length(X_all.ln)));
    H.xls(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,4)
    [F, c1, c2] = smoothhist2D_corrected([C_all.ln' X_all.ln'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.ln)/(length(X_all.ls)+length(X_all.ln)));
    H.xln(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,7)
    [F, c1, c2] = smoothhist2D_corrected([C_all.la' X_all.la'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.la)/(length(X_all.la)+length(X_all.la)));
    H.xla(iseries,:,:) = F; %./ mean(F(:));
    
    subplot(3,3,2)
    [F, c1, c2] = smoothhist2D_corrected([C_all.ns' X_all.ns'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.ns)/(length(X_all.ns)+length(X_all.nn)));
    H.xns(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,5)
    [F, c1, c2] = smoothhist2D_corrected([C_all.nn' X_all.nn'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.nn)/(length(X_all.ns)+length(X_all.nn)));
    H.xnn(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,8)
    [F, c1, c2] = smoothhist2D_corrected([C_all.na' X_all.na'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.na)/(length(X_all.na)+length(X_all.na)));
    H.xna(iseries,:,:) = F; %./ mean(F(:));
    
    subplot(3,3,3)
    [F, c1, c2] = smoothhist2D_corrected([C_all.hs' X_all.hs'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.hs)/(length(X_all.hs)+length(X_all.hn)));
    H.xhs(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,6)
    [F, c1, c2] = smoothhist2D_corrected([C_all.hn' X_all.hn'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.hn)/(length(X_all.hs)+length(X_all.hn)));
    H.xhn(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,9)
    [F, c1, c2] = smoothhist2D_corrected([C_all.ha' X_all.ha'],4,[50 50], 1:50);
%     F = smoothhist2D_corrected([X_all.ha' C_all.ha'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(X_all.ha)/(length(X_all.ha)+length(X_all.ha)));
    H.xha(iseries,:,:) = F; %./ mean(F(:));
    
    subplot(3,3,1)
    [F, c1, c2] = smoothhist2D_corrected([V_all.as' X_all.as'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.ls)/(length(V_all.ls)+length(V_all.ln)));
    H.as(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,4)
    [F, c1, c2] = smoothhist2D_corrected([V_all.an' X_all.an'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.ln)/(length(V_all.ls)+length(V_all.ln)));
    H.an(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,7)
    [F, c1, c2] = smoothhist2D_corrected([V_all.aa' X_all.aa'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.la)/(length(V_all.la)+length(V_all.la)));
    H.aa(iseries,:,:) = F; %./ mean(F(:));
    
    subplot(3,3,1)
    [F, c1, c2] = smoothhist2D_corrected([C_all.as' X_all.as'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.ls)/(length(V_all.ls)+length(V_all.ln)));
    H.xas(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,4)
    [F, c1, c2] = smoothhist2D_corrected([C_all.an' X_all.an'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.ln)/(length(V_all.ls)+length(V_all.ln)));
    H.xan(iseries,:,:) = F; %./ mean(F(:));
    subplot(3,3,7)
    [F, c1, c2] = smoothhist2D_corrected([C_all.aa' X_all.aa'],4,[50 50], 1:50);
    imagesc(c1, c2,F*2500 * length(V_all.la)/(length(V_all.la)+length(V_all.la)));
