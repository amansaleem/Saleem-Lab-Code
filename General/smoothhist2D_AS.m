function [F, ctrs1, ctrs2, H] = smoothhist2D_AS(X,lambda,nbins, bins,outliercutoff,plottype)
...
if nargin < 5 || isempty(outliercutoff), outliercutoff = .05; end
if nargin < 6, plottype = 'none'; end

edges1 = bins;
ctrs1 = edges1(1:end-1) + .5*diff(edges1);
edges1 = [-Inf bins(2:end) Inf];
edges2 = bins;
ctrs2 = edges2(1:end-1) + .5*diff(edges2);
edges2 = [-Inf bins(2:end-1) Inf];
