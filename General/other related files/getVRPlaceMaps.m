function [rateMap I_skaggs R traj spikeRate normTraj posXY] = getVRPlaceMaps(animal,series,iexp, discreet_steps, replay_in)
...
for ichan = 1:size(spikeRate,1)
    for iprot = 1:size(spikeRate,2)
        if ~isempty(spikeRate{ichan,iprot})
            for iplace = 1:length(places)
                for itrial = 1:length(trials)
                    rateMap(ichan,iprot).spikeRate{iplace,itrial} = (spikeRate{ichan,iprot}(normTraj==places(iplace) & trialIdx==trials(itrial)));
                    rateMap(ichan,iprot).rate(iplace,itrial)  = mean(rateMap(ichan,iprot).spikeRate{iplace,itrial});
                    rateMap(ichan,iprot).P_place(iplace,itrial)   = sum((normTraj==places(iplace)) & (trialIdx==trials(itrial)));
                end
            end
            % To calculate Skagg's info of each neuron
            rateMap(ichan,iprot).meanRate = nanmean(rateMap(ichan,iprot).rate, 2);
            rateMap(ichan,iprot).Lambda = nanmean(rateMap(ichan,iprot).meanRate);
...
%                 ylabel('Firing rate (spks/s)', 'fontsize', 14);
            ...
            for iplace = 1:length(places)
                rateMap(ichan,iprot).spikeRate{iplace} = [];
            end
            rateMap(ichan,iprot).rate  = [];
            rateMap(ichan,iprot).P_place   = [];
        end
