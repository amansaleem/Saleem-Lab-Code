function [EV, kernels] = backslash_cv(ianimal, iseries, inputs2use)
% close all;
% clear;
% clc;
...
k = load([num2str(ianimal) '_' num2str(iseries) '_freq_V1_CA1']);
if ~isfield(k, 'es')
...
for iter = 1:kfold
    ...
    if addBiasTerm
        ...
        outputs = mat2cell(squeeze(kernels(iter,:,:)), [ones(length(inputs2use), 1)*(order+1)], size(kernels, 3));
    end
